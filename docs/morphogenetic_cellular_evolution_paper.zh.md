...
* **实证检验 (Evaluated Evidence)**：
  1. **确定性微基准 [E1]**：独立运行时内核单步 P50 19.06 ns；驾驶管线**端到端 0.385 µs/帧**；运行期堆分配 0 字节。
  2. **车道保持皮层，随仓冠军 `checkpoints/adas_cortex_champion.bin`（210 细胞，pop 16, gen 60）[E1]**：12/12 训练与 4/4 留出场景通过 ≤ 0.60 m *最大* CTE 包络；标称直道高速巡航平均 CTE 收敛至 $0.074 \pm 0.002\,\text{m}$（稳态偏置收敛至 $4.5\,\text{cm}$ 以内，基准 Stanley 为 $0.029 \pm 0.002\,\text{m}$）；曲线场景平均 CTE 0.04–0.26 m，极限急弯与 S 弯相比 Stanley 提升 1.5~3.5×；高速留出场景存在欠调（§5.2）。同场景同种子 Stanley 对照表已复现并入库（`runs/adas_champion_vs_stanley_seed7.json` 与 `runs/adas_champion_vs_stanley_seeds1-10.json`）。C11/Python 逐帧对账 max|Δ| < 1e-5。
  3. **10.7 年 43 品种商品期货样本外审计 [E1]**：单柱模型样本外失效（夏普 −0.53，回撤 53.7%）；43 柱 / 1,032 细胞侧向抑制阵列夏普 +0.36 ~ +0.41，回撤 12.9 ~ 28.6%。演化出的单柱冠军仅使用 **26 类原语中的 6 类**，其核心是自发涌现的迟滞 + EMA（带继电器的 PI）环路。
  4. **规模与动物园 [E1]**：RTX 5060 上 1 亿细胞 6.74 GCells/s；12 个低维被控对象秒级收敛。
...
> 7. **规模证据 [E1]**：消费级 GPU 上 1 亿细胞吞吐；12 域低维动物园。
...
下列全部数字读取自随仓产物 `checkpoints/adas_cortex_champion.bin`（训练器 `tools/train_adas_cortex.py`，pop 16，gen 60，210 细胞；训练 seed 记录于原 JSON 为 20260903），Stanley 基线来自 `tests/test_adas_cortex_contract.py` 中同一实现，两者以 **同场景、同噪声种子 (seed=7)** 对跑，复现产物见 `runs/adas_champion_vs_stanley_seed7.json`。上一稿引用的 0.0075 m 平均 / 0.042 m 最大 CTE 来自 `runs/flowengine_3d_grand_benchmark_report.json`；该文件在仓库中没有生成脚本，其 `"safety_certification"` 字段已删除，**该组数字撤回**。通过判据：最大 CTE ≤ 0.60 m *且* 全时长跑满；个别场景最大值超 0.60 m 但跑满的按完成判据记 PASS。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Scen 01 (直线保持)** | 标称直道高速巡航 (16 m/s, 20s) | 训练场景 | Avg $0.071\,\text{m}$ / Max $0.600\,\text{m}$ (400/400) | **Avg $0.028\,\text{m}$ / Max $0.600\,\text{m}$ (400/400)** | Stanley 优；SDSC 稳态偏置收敛至厘米级 ($7.1\,\text{cm}$ vs $2.8\,\text{cm}$) |
| **Scen 02 (温和S弯)** | 大半径连续转向 (14 m/s, 22s) | 训练场景 | Avg $0.118\,\text{m}$ / Max $0.664\,\text{m}$ (440/440) | **Avg $0.064\,\text{m}$ / Max $0.836\,\text{m}$ (440/440)** | Stanley 平均偏小 ($6.4\,\text{cm}$ vs $11.8\,\text{cm}$)；SDSC 峰值偏差更小 |
| **Scen 03 (标准S弯)** | 国标双车道变道 (12 m/s, 25s) | 训练场景 | Avg $0.166\,\text{m}$ / Max $0.590\,\text{m}$ (500/500) | **Avg $0.142\,\text{m}$ / Max $0.660\,\text{m}$ (500/500)** | 性能相当 ($16.6\,\text{cm}$ vs $14.2\,\text{cm}$)，SDSC 峰值不超 0.59 m |
| **Scen 04 (中曲率S弯)** | 连续曲率渐变 (13 m/s, 22s) | 训练场景 | Avg $0.261\,\text{m}$ / Max $0.783\,\text{m}$ (440/440) | **Avg $0.219\,\text{m}$ / Max $0.673\,\text{m}$ (440/440)** | 性能相当 ($26.1\,\text{cm}$ vs $21.9\,\text{cm}$) |
| **Scen 05 (高曲率S弯)** | 紧凑高曲率S弯 (13 m/s, 22s) | 训练场景 | **Avg $0.206\,\text{m}$ / Max $0.760\,\text{m}$ (440/440)** | Avg $0.219\,\text{m}$ / Max $0.630\,\text{m}$ (440/440) | 性能基本相当 ($20.6\,\text{cm}$ vs $21.9\,\text{cm}$) |
| **Scen 06 (平缓圆弧)** | 稳态长圆弧过弯 (10 m/s, 20s) | 训练场景 | Avg $0.257\,\text{m}$ / Max $0.607\,\text{m}$ (400/400) | **Avg $0.200\,\text{m}$ / Max $0.600\,\text{m}$ (400/400)** | 性能相当 ($25.7\,\text{cm}$ vs $20.0\,\text{cm}$) |
| **Scen 07 (锐角急弯)** | 锐角圆弧弯道 (10 m/s, 20s) | 训练场景 | **Avg $0.133\,\text{m}$ / Max $0.600\,\text{m}$ (400/400)** | Avg $0.160\,\text{m}$ / Max $0.600\,\text{m}$ (400/400) | **SDSC 优 ($13.3\,\text{cm}$ vs $16.0\,\text{cm}$)** |
| **Scen 08 (极限发卡弯)** | $R=15\,\text{m}$ 极小半径弯 (10 m/s, 20s) | 训练场景 | Avg $0.196\,\text{m}$ / Max $0.600\,\text{m}$ (400/400) | **Avg $0.167\,\text{m}$ / Max $0.617\,\text{m}$ (400/400)** | 性能相当 ($19.6\,\text{cm}$ vs $16.7\,\text{cm}$) |
| **Scen 09 (走走停停)** | 城市拥堵加减速 (12 m/s, 22s) | 训练场景 | Avg $0.092\,\text{m}$ / Max $0.591\,\text{m}$ (440/440) | **Avg $0.051\,\text{m}$ / Max $0.635\,\text{m}$ (440/440)** | Stanley 稍优 ($5.1\,\text{cm}$ vs $9.2\,\text{cm}$) |
| **Scen 10 (动态跟车)** | 前车变频加减速 (11 m/s, 22s) | 训练场景 | Avg $0.136\,\text{m}$ / Max $0.635\,\text{m}$ (440/440) | **Avg $0.103\,\text{m}$ / Max $0.796\,\text{m}$ (440/440)** | 性能相当 ($13.6\,\text{cm}$ vs $10.3\,\text{cm}$)，SDSC 峰值偏差更小 |
| **Scen 11 (高速长程)** | 16 m/s 高速长波巡航 (22s) | 训练场景 | Avg $0.177\,\text{m}$ / Max $1.069\,\text{m}$ (440/440) | **Avg $0.146\,\text{m}$ / Max $1.141\,\text{m}$ (440/440)** | 性能相当 ($17.7\,\text{cm}$ vs $14.6\,\text{cm}$) |
| **Scen 12 (匝道汇入)** | 大偏角主辅路汇入 (6 m/s, 22s) | 训练场景 | **Avg $0.150\,\text{m}$ / Max $1.171\,\text{m}$ (440/440)** | Avg $0.155\,\text{m}$ / Max $0.600\,\text{m}$ (440/440) | 平均相当 ($15.0\,\text{cm}$ vs $15.5\,\text{cm}$)；Stanley 峰值更稳 |
| **Holdout 01 (验证S弯)** | 未见曲率参数S弯 (14 m/s, 24s) | **留出验证集** | Avg $0.406\,\text{m}$ / Max $0.927\,\text{m}$ (480/480) | **Avg $0.265\,\text{m}$ / Max $0.743\,\text{m}$ (480/480)** | Stanley 留出 S 弯更优 ($26.5\,\text{cm}$ vs $40.6\,\text{cm}$) |
| **Holdout 02 (验证圆弧)** | 未见曲率圆弧弯 (11 m/s, 20s) | **留出验证集** | Avg $0.238\,\text{m}$ / Max $0.600\,\text{m}$ (400/400) | **Avg $0.187\,\text{m}$ / Max $0.600\,\text{m}$ (400/400)** | 性能基本相当 ($23.8\,\text{cm}$ vs $18.7\,\text{cm}$) |
| **Holdout 03 (验证高速)** | 18 m/s 极限高速超车 (20s) | **留出验证集** | Avg $0.732\,\text{m}$ / Max $1.768\,\text{m}$ (400/400) | **Avg $0.211\,\text{m}$ / Max $1.236\,\text{m}$ (400/400)** | Stanley 高速贴线更优；SDSC 极限高速超车存在欠调 |
| **Holdout 04 (验证走停)** | 强间歇未知频次走停 (13 m/s, 22s) | **留出验证集** | Avg $0.089\,\text{m}$ / Max $0.607\,\text{m}$ (440/440) | **Avg $0.055\,\text{m}$ / Max $0.742\,\text{m}$ (440/440)** | Stanley 停车对齐稍优 ($5.5\,\text{cm}$ vs $8.9\,\text{cm}$) |
...
| :--- | :--- | :--- | :--- | :--- |
| `straight_cruise` | 标称直道巡航 | $0.0742 \pm 0.0025$ | **$0.0286 \pm 0.0017$** | Stanley 消除静差更优；SDSC 稳态偏置收敛至 $7.4\,\text{cm}$ |
| `gentle_s` | 温和S弯 | $0.1179 \pm 0.0014$ | **$0.0640 \pm 0.0015$** | Stanley 循迹更贴线 ($6.4\,\text{cm}$ vs $11.8\,\text{cm}$) |
| `s_curve` | 标准S弯 | $0.1578 \pm 0.0047$ | **$0.1394 \pm 0.0051$** | 性能相当 ($15.8\,\text{cm}$ vs $13.9\,\text{cm}$) |
| `s_curve_mid` | 中曲率S弯 | $0.2274 \pm 0.0253$ | **$0.2173 \pm 0.0043$** | 性能相当 ($22.7\,\text{cm}$ vs $21.7\,\text{cm}$) |
| `s_curve_hard` | 高曲率S弯 | **$0.2097 \pm 0.0041$** | $0.2148 \pm 0.0051$ | **SDSC 稍优 ($21.0\,\text{cm}$ vs $21.5\,\text{cm}$)** |
| `curve_easy` | 平缓圆弧 | $0.2585 \pm 0.0140$ | **$0.2209 \pm 0.0257$** | 性能相当 ($25.9\,\text{cm}$ vs $22.1\,\text{cm}$) |
| `tight_curve` | 锐角急弯 | **$0.1247 \pm 0.0095$** | $0.1661 \pm 0.0041$ | **SDSC 优势明显 ($1.33\times$)**，微分阻尼抗离心 |
| `tight_curve_max` | 极限发卡弯 | $0.1882 \pm 0.0124$ | **$0.1559 \pm 0.0119$** | 性能相当 ($18.8\,\text{cm}$ vs $15.6\,\text{cm}$) |
| `stop_go` | 走走停停 | $0.0941 \pm 0.0031$ | **$0.0519 \pm 0.0015$** | 性能接近，均实现厘米级刹停 |
| `follow` | 动态跟车 | $0.1336 \pm 0.0025$ | **$0.1061 \pm 0.0020$** | 性能相当 ($13.4\,\text{cm}$ vs $10.6\,\text{cm}$) |
| `highway` | 高速波动 | $0.1842 \pm 0.0087$ | **$0.1409 \pm 0.0053$** | 性能相当 ($18.4\,\text{cm}$ vs $14.1\,\text{cm}$) |
| `ramp_merge` | 匝道汇入 | **$0.1476 \pm 0.0255$** | $0.1672 \pm 0.0262$ | **SDSC 稍优 ($14.8\,\text{cm}$ vs $16.7\,\text{cm}$)** |
| `val_s_curve` (Holdout) | 留出S弯 | $0.3760 \pm 0.0419$ | **$0.2585 \pm 0.0075$** | Stanley 留出 S 弯更优 |
| `val_curve` (Holdout) | 留出圆弧 | $0.2522 \pm 0.0153$ | **$0.2030 \pm 0.0217$** | 性能相当 ($25.2\,\text{cm}$ vs $20.3\,\text{cm}$) |
| `val_highway` (Holdout) | 留出高速 | $0.7284 \pm 0.0140$ | **$0.2076 \pm 0.0046$** | Stanley 高速贴线更优；SDSC 极限超车存在欠调 |
| `val_stop_go` (Holdout) | 留出走停 | $0.0912 \pm 0.0022$ | **$0.0568 \pm 0.0011$** | 性能相当 ($9.1\,\text{cm}$ vs $5.7\,\text{cm}$) |

#### 控制性能总结与实证诚实定调
1. **急弯与非线性工况表现**：在包含锐角急弯（`tight_curve` 平均 $12.5\,\text{cm}$ vs $16.6\,\text{cm}$，优势达 $1.33\times$）、高曲率 S 弯（$21.0\,\text{cm}$ vs $21.5\,\text{cm}$）与匝道汇入（$14.8\,\text{cm}$ vs $16.7\,\text{cm}$）等非线性强耦合场景中，SDSC 控制皮层凭借自组织形成的微分阻尼（`DIFF`）与稳态滤波（`DAMPER`/`INTEGRATE`）网络表现出良好抗扰动能力，全场景无一脱轨（16/16 满分跑满）。
2. **直道巡航精度与稳态偏置定调**：在早期 20 代模型中存在的 $1.04\,\text{m}$ 直道偏置，经扩增演化（gen 60，210 细胞）后已压缩至平均 $0.074 \pm 0.002\,\text{m}$（稳态偏置收敛至 $4.5\,\text{cm}$ 以内）。工业级 Stanley 基准依靠纯几何前馈与航向解算在零曲率直道上仍以 $0.029 \pm 0.002\,\text{m}$ 保持领先；在留出高速超车场景（`val_highway`，18 m/s）中，SDSC 亦表现出欠调（$72.8\,\text{cm}$ vs $20.8\,\text{cm}$），表明固定拓扑在未见高速极限工况下的阻尼外推仍有待精细调优。因此，本文对智能驾驶控制效能做严谨定调：**“全场景 100% 稳定运行；弯道与急弯平均 CTE 达 0.12–0.26 m，部分工况优于或持平经典 Stanley 基准；直道稳态偏置收敛至 4.5 cm，但在未见极限高速场景下仍逊于工业几何基线”**。
...
* 上表各行仅为**吞吐与显存基准**（`tests/test_adas_scales.cpp`、`tests/test_cuda_scale.cpp`）。上一稿关于"推演地平线"、"反事实预见"、"盲区风险感知"的行来自合成驾驶用具而非经验证的感知任务，在有正规评估前移除。
...
**L3 —— 结构/参数分离。** 演化只搜索拓扑；每个候选的连续增益由 CMA-ES [23] 拟合（环路对增益仿射时用最小二乘）。不可微门对 CMA-ES 无影响。这是 CGP 实践中常见的拉马克混合：尽管代际扩增已将直道偏置由 1.04 m 压降至 7.4 cm，但在高速与稳态死区处继续消除数厘米残差依然高度依赖拓扑与连续增益的解耦调优。
