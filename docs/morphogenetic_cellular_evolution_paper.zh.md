...
* **核心方法 (Method)**：本文提出一种**函数集固定为 28 类控制论原语的笛卡尔遗传编程 (CGP) 变体**（EMA/积分器、微分器、施密特迟滞、死区、饱和、相关、Van der Pol 型振荡器、疲劳、效应器），其权威清单以 `include/kun/cellular/ops.yaml` (v2.0.0) 为准。拓扑与参数由形态发生算子（有丝分裂、轴突重连、凋亡）演化。两道搜索空间门控约束演化：**环路增益筛查**（Tarjan SCC + 增益乘积，含显式耗散门豁免）与**应变选址的发育编码**（Lennard-Jones 布局场决定有丝分裂*发生在哪*）。冠军图经 Kahn 排序为连续 CSR 数组，导出为 **SDSC-BIN (v2)** 与零分配 C11。
* **实证检验 (Evaluated Evidence)**：
  1. **确定性微基准 [E1]**：100 万细胞 / 300 万突触生命体（`checkpoints/sdsc_mega_1million.bin`）整步 CSR 前向推演耗时 **134.588 ms（7.43 MCells/s）**，mmap 零拷贝挂载 5.328 ms（`tests/test_binary_runtime_scale.c`）；运行期堆分配 0 字节。*此前宣称的 19.06 ns P50 / 52.47 M-Inferences/s / 端到端 0.385 µs 已撤回——其来源（`test_c11_apex_maneuver`、`sdsc_apex_cortex.h`）已于提交 `b555cb7` 中移除，当前代码树无法复现。*
  2. **车道保持皮层，随仓冠军 `checkpoints/adas_cortex_champion.bin`（210 细胞，630 突触）[E1]**：12/12 训练与 4/4 留出场景 100% 满分通过（≤ 0.60 m 最大 CTE 包络）；标准 S 弯平均 CTE 6.36 cm，温和 S 弯 6.03 cm，极限急弯 3.96 cm，走走停停 5.15 cm，留出验证集走走停停 7.76 cm；留出泛化比仅 0.29x；直道巡航经 L3 参数调优后稳态偏置压缩至 3.38 cm（$0.0338 \pm 0.0015\,\text{m}$，基准 Stanley 为 $0.0286 \pm 0.0017\,\text{m}$，差距仅 5.2 mm）；在其胜出的场景（s_curve、curve_easy、tight_curve、highway、ramp_merge）上优于 Stanley 1.0~1.3x，但 16 场景总战绩为 7 胜 9 负（表 2b）；C11/Python 逐帧对账达成位级一致（max |Δ| < 1e-5，PASS）。同场景同种子对照数据见 `runs/adas_champion_vs_stanley_seed7.json` 与 `runs/adas_champion_vs_stanley_seeds1-10.json`。**口径更新（2026-09-11）**：随仓冠军经 L3 sep-CMA-ES 重调后为 210 细胞 / **659 突触**、16 场景 **9 胜 7 负**（`runs/adas_champion_vs_stanley_seeds1-10_tuned_20260911.json`）；本节表格保留 2026-09-04 审计快照（630 突触 / 7 胜 9 负），当前口径以 README 为准。
  3. **10.7 年 43 品种商品期货样本外审计 [E1]**：单微柱玩具模型因缺乏空间侧向抑制而在样本外严重过拟合崩溃（10 年盲测夏普 -0.28 ~ -0.53）；43 微柱宏阵列的论文锚点曾报年化夏普 0.22 / +13.55% / 卡玛 0.67，**`--seed 0` 确定性复训未复现**（选择集夏普 2.16，OOS 夏普 **-0.03**；`checkpoints/quant_cortical_array_oos_repro_20260911.json`）。正式 `quant_cortical_array_champion.bin` 任务层冷评（`eval_cortical_array_bin`，u8/64 存盘表型）OOS 夏普 **+0.14** / +6.3% / 回撤 25.6%，**仍不是**锚点 0.22。此前 +20.30% / 夏普 +0.36 已撤回。Gate 3 对「可复现 0.22」**锁 FAIL**。
  4. **全尺度硬件与物理动物园 [E1]**：单张消费级 RTX 5060 (8GB) 上 1 亿细胞纯算子核推演吞吐达 916.3 ~ 1,027.9 GCells/s（0.097 ~ 0.109 ms/step，显存仅 2.76 GB，全闭环 14.8 ms / 6.74 GCells/s）；DomainZoo 12 大经典物理动力学被控系统在确定性重跑（`tools/train_domain_zoo.cpp`）中 **12/12 通过 M1 门禁**（`checkpoints/domain_zoo_report.json`）；2026-09-09 曾短暂回落为 9/12（maglev / dc_motor / servo），经 Task 层植物/课程修复后于 2026-09-10/11 及 2026-09-14 复跑锁回 12/12，不得再引用过期 9/12 作为当前状态；空间迷宫死胡同寻优达成 100% 逃逸与 0 死锁。
...
> 1. **面向 CGP 的控制论原语函数集 [E2]**：28 类带类型、带状态、具显式传递方程的原语（表 1），使搜索空间天然包含 PID / 继电器 / 超前滞后等积木——演化因此是*重新发现*控制器而非从加权求和里重新发明。
...
> 6. **含负面基线的诚实多资产审计 [E1]**：10.7 年样本外评估；`--seed 0` 复训 OOS **-0.03**；正式 bin 冷评 **+0.14**（不是锚点 0.22）；不得宣称穿越宏观相变。
> 7. **全尺度硬件吞吐、六道实证门禁与物理动物园 [E1]**：确立由探针、收敛、OOD盲测、确定性零GC、离线回放与影子对账组成的六道实证门禁；在消费级 GPU 上达成 1 亿细胞超千 GCells/s 吞吐；覆盖 DomainZoo 12 域低维动力学物理系统（现仓确定性重跑 **12/12** 通过；中间窗 9/12 已勘误）与空间迷宫 100% 逃逸。
...
(iii) **固定完备基元先验**：正如地球碳基生命严格依赖 22 种标准蛋白质编码氨基酸（含硒代半胱氨酸与吡咯赖氨酸）完成全部生物化学反应，CPS 控制器亦可建立在一组固定的 28 类原子控制论动力学原语之上。
...
graph LR
    ...
    subgraph 本工作: 控制原语 CGP
      C1[28 类带类型带状态原语] --> C2[环路增益筛查 + 应变选址有丝分裂] --> C3[Kahn/CSR -> SDSC-BIN v2 -> 零GC C11, 零堆分配内核]
    end
```
...
## 3. 系统模型与 28 类原子原语分类学 (System Model & 28 Primitives)
...
其中：
* $\tau_i \in \{0, 1, \dots, 27\}$：完备原子动力学原语类型标识；
...
### 表 1：完备 28 类原子计算动力学原语分类学与传递函数 [E2]
...
| **代谢运算族**<br>(Accumulator) | `SDSC_OP_ACCUMULATOR` (27) | $s_i^{(t)} = \text{clamp}(s_i^{(t-1)} + x_i^{(t)} \cdot g_i, -16.0, 16.0), \quad u_i^{(t)} = s_i^{(t)}$ | **有界持久累加寄存器**（区间饱和积分，时序工作记忆核） |

#### 原语选择的实证检验与先验抽样边界
虽然底座形式化支持 28 类完备原子原语，但在实证演化中呈现出鲜明的任务维度分化：
...
2. **高维控制皮层的初始化先验客观定调**：在 192 隐藏细胞的 ADAS 驾驶皮层（`adas_cortex_champion.bin`）中，实测呈现出 18 种原语每种 6~19 个的近似均匀分布。代码溯源表明（`train_adas_cortex.py:190`），该分布由初始化的伪随机抽样（`random.choice`）决定，在当前 `pop=16, gen=20` 的较浅演化窗口中，演化算子尚未充分施加类型重塑压力。因此该 18 类多样性属于抽样先验，不可误读为“演化自发利用了 18 类协同原语”；其余 18 种原语在 28 原语库中的不可替代性仍待深度代际选择压力进一步实证检验。
...
typedef struct {
    uint8_t  op_type;          /* 28 原语算子枚举 (0~27) */
...
4. **门禁 4：纯 C 原生确定性零 GC 与零堆分配认证 (Deterministic Zero-GC)**：前向执行期堆分配必须严格为 0 字节，且相同输入必须产生位级可复现（确定性）输出；
...
### 5.1 确定性零 GC 推理微架构 [E1]
在标准商用 x86-64 处理器上可复现的实测（`tests/test_binary_runtime_scale.c`），加载 `checkpoints/sdsc_mega_1million.bin`（1,000,000 细胞 / 3,000,000 突触，64 受体 / 16 效应器），零拷贝 mmap：
* **mmap 挂载耗时**：**5.328 ms**；
* **百万细胞整步前向推演**：**134.588 ms**；
* **吞吐速率**：**7.43 MCells/sec**；
* **执行期内存分配**：全程严格为 **0 字节堆分配（0 malloc / 0 free）**，无 GC 停顿。
* **已撤回**：此前的 **19.06 ns P50 / 52.47 M-Inferences/sec / 179.0 ns P99 / 35.9 µs 最坏延迟** 来自 `test_c11_apex_maneuver` 与 `include/kun/cellular/sdsc_apex_cortex.h`，二者已随提交 `b555cb7` 移除；当前代码树中无任何产物可复现这些数字，不得引用。

### 5.2 具身智能驾驶控制皮层 16 大场景闭环实测、Stanley 基线对比与 C11 逐帧对账 [E1]
在 2026 年 9 月的全流程亲手复测中，使用 `tools/train_adas_cortex.py` 从零启动代际演化（pop 16, gen 20, 210 细胞, 630 突触），12 个训练场景与 4 个留出验证场景达成 **100% 满分通过**；标准 S 弯 CTE 为 $6.36\,\text{cm}$，温和 S 弯 $6.03\,\text{cm}$，极限急弯 $3.96\,\text{cm}$，走走停停 $5.15\,\text{cm}$，留出验证走走停停 $7.76\,\text{cm}$，留出集泛化差距比仅 $0.29\times$；导出的 C11 标头与 Python 逐帧推演达成绝对位级一致（$\max \vert \Delta \vert < 10^{-5}$，`tests/test_adas_cortex_parity.py` 2/2 PASS）。
...
| **43 微柱皮层宏阵列**<br>(`CorticalMacroArray`) | **43 微柱 / 1,032 细胞** | `--seed 0` 复训选择集夏普 **2.16** / +85.3% / 回撤 5.6%；正式 bin 冷评 val **0.74** / +15.7% | 复训 OOS **-0.03**；正式 bin 冷评 OOS **+0.14** / +6.3% / 回撤 25.6%（2593 日） | **Gate 3 对锚点 0.22 FAIL**。复训过拟合已冻结；正式档可冷评但不是 0.22。证据 `quant_cortical_array_oos_repro_20260911.json` 与 `quant_cortical_array_champion_cold_eval_20260914.json` |

* **学术发现**：单一微柱盲目加代数会样本内记忆化。43 柱 `--seed 0` 复训再现选择集繁荣（夏普 2.16）与 OOS 崩溃（-0.03）。正式 `champion.bin` 现可任务层冷评，OOS 夏普 **+0.14**，**仍不得把 0.22 当作当前成绩**。
...
为验证 SDSCC 在经典物理与控制理论中的普适通用性，我们在 `tasks/control/domain_zoo.hpp` 中构建了覆盖工业控制与力学运动学的 12 大经典基准，各域均秒级收敛并生成紧凑 `.bin` 镜像。以 `tools/train_domain_zoo.cpp` 确定性重跑（2026-09-14 复跑锁档，报告 `checkpoints/domain_zoo_report.json`），**12/12 域全部通过 M1 门禁**。勘误史：2026-09-09 曾因报告与 bin 漂移回落为 9/12（MagLev / DC Motor / Servo）；2026-09-10 在 **Task 层**修复被控对象与课程后对齐为 12/12——**不得把中间窗 9/12 当作当前状态**：
...
6. **DC Motor (直流电机转速与位置伺服)**：方波脉冲跟随上升时间 $< 50\,\text{ms}$（中间窗曾 FAIL，Task 层植物修复后锁 PASS）；
7. **MagLev (主动磁悬浮轴承)**：强开环不稳定系统在 $1\,\text{mm}$ 间隙处实现坚硬吸附（中间窗曾 FAIL，课程复用后锁 PASS）；
8. **Rocket Hover (火箭垂直回收反推悬停)**：主推进器推力矢量与姿态喷气微调，软着陆速度 $< 0.1\,\text{m/s}$；
9. **Servo (高精度机械舵机)**：快速阶跃响应，死区门控有效消除极限环抖振（中间窗曾 FAIL，Task 层修复后锁 PASS）；
...
* **实证检验（已撤回并替换）**：1,024 细胞 / 196,608 突触的冠军模型（`checkpoints/doudizhu_game_champion.bin`，拓扑已与其 `.cert.json` 核对）此前被记为达成 **82.5%** 样本外对弈胜率；**该数字已撤回**——其训练与测试环境为简化合成仿真（无叫分、无合法牌型判定、对手为随机数发生器），对真实三人斗地主无参考价值。当前如实报告的基线为 U0.2 冠军 `checkpoints/doudizhu_cand_scorer.bin`（82 细胞 / 425 突触）：**胜率 57.0%（1139/2000 局，Wilson 95% 下界 54.8%）**，对手为启发式策略、使用完整规则引擎（地主 61.2% / 农民 56.8%）。此前报告的 **411.8 μs** 单步时延一并撤回，其测量产物已不在代码树中。

---

## 6. 有效性威胁与局限性说明 (Threats to Validity)

1. **撤回的头条数字**：上一稿 0.0075 m / 0.042 m CTE 与 "ISO 26262 ASIL-D Compliant" 字符串来自不可复现的报告文件；**19.06 ns P50 / 52.47 M-Inferences/sec** 微基准来自 `test_c11_apex_maneuver` 与 `sdsc_apex_cortex.h`，二者已随提交 `b555cb7` 移除，当前代码树中不存在。表 2 现仅使用随仓冠军，§5.1 仅报告 `tests/test_binary_runtime_scale.c` 可复现的数字。本文任何内容都不构成功能安全认证；全部驾驶结果仅为仿真。
...
5. **原语集必要性**：量化冠军使用 28 类中的 6 类；ADAS 18 类直方图是初始化先验。其余原语的必要性未证明。
6. **四算子消融**：§5.9 四个启发式仍未做开关。2026-09-15 在 ZooCartPole 上对**默认 mutate 旋钮**做了任务层消融（`ablate_morph_operators`）：加边/重连承重（ID 1.00→0.43），有丝分裂与鲍德温不承重；凋亡仍无法关闭。不得把该表写成 §5.9 已完成。
...
12. **量化宏阵列与 DomainZoo 数字勘误**：43 柱阵列 +20.30% / 夏普 +0.36 已撤回；论文锚点夏普 0.22 在 `--seed 0` 复训中 **未复现**（OOS **-0.03**）。正式 bin 冷评 OOS **+0.14**，不是 0.22。DomainZoo 现仓 **12/12**（§5.5）。
13. **拓扑因果显著性与消融对照门禁**：受全脑连接组对照实验 [29] 启发，已在任务层落地 **同度数拓扑随机重连（Degree-Preserving Random Rewiring）** 与 **兴奋/抑制符号翻转（E/I Flipping）**（`tools/ablate_topology_prior.py`，稳定性战役 #4）。ADAS L3 锁档与体育场域适配档均 **PASS**：重连代价分别为锁档的 34.1× / 23.9×（8 次重连完赛率 0），符号翻转 10.8× / 22.0×。证据：`runs/adas_topology_ablation_20260914.json`。目前仅覆盖 ADAS 生境。
...
28 原语集合类比于固定指令集：需要新程序时不会给 ISA 增加 `LaneKeep` 操作码。§5 全部结果均仅通过改变任务层的*适应度函数、传感编码与训练循环*获得；提交 `c19f645` 正是为此把所有任务头文件迁出 `include/kun/cellular/`。这个类比有两条必须写明的边界：
...
**SDSCC 形态发生演化**：
- 演化对象：**28 种异构原子动力学原语的 DAG 拓扑图谱本身**；
...
| **万物**（森罗万象） | 28 种原子原语拓扑涌现 | 从车道居中到多资产截面宏阵列对冲，均从同一组原语中自发涌现 |
