...
* **实证检验 (Evaluated Evidence)**：
  ...
  3. **10.7 年 43 品种商品期货样本外审计 [E1]**：单微柱玩具模型因缺乏空间侧向抑制而在样本外严重过拟合崩溃（10 年盲测夏普 -0.28 ~ -0.53，累计收益 -18.02% ~ -35.72%，最大回撤 31.48% ~ 53.71%）；而 43 微柱 / 1,032 细胞皮层宏阵列（258 条跨柱侧向抑制长程轴突）达成**年化夏普 0.22，10 年盲测累计收益 +13.55%（100 万 -> 113.5 万），最大回撤 20.11%，卡玛比率 0.67**（2,593 个交易日，1,141 次截面调仓），由 `tools/train_multi_asset_cortical_array.cpp` 复现。*此前的 +20.30% / 夏普 +0.36 / 卡玛 1.58 不可复现，已撤回。* 演化出的单柱微观回路自发收敛于 6 类核心原语（迟滞 + EMA 稳态滤波）。
  4. **全尺度硬件与物理动物园 [E1]**：单张消费级 RTX 5060 (8GB) 上 1 亿细胞纯算子核推演吞吐达 916.3 ~ 1,027.9 GCells/s（0.097 ~ 0.109 ms/step，显存仅 2.76 GB，全闭环 14.8 ms / 6.74 GCells/s）；DomainZoo 12 大经典物理动力学被控系统在确定性重跑（`tools/train_domain_zoo.cpp`）中 9/12 通过 M1 门禁（maglev / dc_motor / servo 未过，如实记录）；空间迷宫死胡同寻优达成 100% 逃逸与 0 死锁。
...
> 7. **全尺度硬件吞吐、六道实证门禁与物理动物园 [E1]**：确立由探针、收敛、OOD盲测、确定性零GC、离线回放与影子对账组成的六道实证门禁；在消费级 GPU 上达成 1 亿细胞超千 GCells/s 吞吐；覆盖 DomainZoo 12 域低维动力学物理系统（确定性重跑 9/12 通过，未过域如实记录）与空间迷宫 100% 逃逸。
...
| **43 微柱皮层宏阵列**<br>(`CorticalMacroArray`, 冠军模型) | **43 微柱 / 1,032 细胞 / 258 跨柱长程抑制轴突** | 训练适应度 4.478；选择集 (2013~2015) 夏普 0.76, 收益 +16.0%, 回撤 7.7% | **年化夏普 0.22, 累计收益率 +13.55% (本金 100万 -> 113.5万), 最大回撤 20.11%, 卡玛比率 0.67（1,141 次调仓）** | **样本外为正但幅度温和**：宏阵列维持了正样本外收益；此前的 +20.30% / 夏普 +0.36 / 卡玛 1.58 不可复现，已撤回 |

* **学术发现**：本实验提供的是方向性的反事实证据——单一微柱由于契约宽度受限，盲目增加训练代数只会导致样本内记忆化；而**皮层微柱宏阵列（Cortical Column Array）**通过 258 条跨资产长程轴突形成全局侧向抑制（Lateral Inhibition），保留了幅度温和但为正的样本外收益（夏普 0.22，卡玛 0.67）。"动态风险预算平衡机制"这一表述缺乏可复现数据支撑，一并撤回。
...
为验证 SDSCC 在经典物理与控制理论中的普适通用性，我们在 `tasks/control/domain_zoo.hpp` 中构建了覆盖工业控制与力学运动学的 12 大经典基准，各域均秒级收敛并生成紧凑 `.bin` 镜像。以 `tools/train_domain_zoo.cpp` 确定性重跑，**12 域中 9 域通过 M1 门禁；MagLev、DC Motor、Servo 未通过（OOD 0%、训练 SR 10%、训练 SR 0%），如实记录为失败**——旧稿的"12/12"来自更早的底座，当前不可复现：
