...
* **Evaluated Evidence**:
  ...
  3. **10.7-year, 43-asset commodity futures out-of-sample audit [E1]**: single-column toy model fails decisively out-of-sample due to lack of lateral inhibition (10-year OOS Sharpe -0.28 to -0.53, return -18.02% to -35.72%, max drawdown 31.48% to 53.71%); in contrast, the 43-column / 1,032-cell cortical macro-array (258 cross-column lateral inhibition axons) achieves **annualized Sharpe 0.22, 10-year OOS cumulative return +13.55% (RMB 1,000,000 -> 1,135,468), max drawdown 20.11%, Calmar 0.67** over 2,593 trading days (1,141 rebalances), reproduced by `tools/train_multi_asset_cortical_array.cpp`. *The earlier +20.30% / Sharpe +0.36 / Calmar 1.58 figures were not reproducible and are withdrawn.* The evolved single-column micro-circuit spontaneously converges to 6 core primitives (Hysteresis + EMA steady-state filter).
  4. **Full-scale hardware & physical zoo [E1]**: 100M-cell pure operator kernel throughput reaches 916.3 ~ 1,027.9 GCells/s (0.097 ~ 0.109 ms/step latency, 2.76 GB VRAM, full pipeline 14.8 ms / 6.74 GCells/s at 67.4 Hz) on a single commodity RTX 5060 (8GB); DomainZoo: 9 of 12 classical physical control domains pass the M1 gate under a deterministic re-run (`tools/train_domain_zoo.cpp`; maglev, dc_motor and servo fail and are reported as such); 2D maze navigation achieves 100% escape with 0 deadlocks and 0 collisions.
...
> 7. **Full-scale hardware throughput, Six Empirical Gates & physical zoo [E1]**: Establishing the Six Empirical Verification Gates protocol (baseline probe, selection convergence, OOD holdout, deterministic zero-GC, offline replay, shadow parity); achieving >1,000 GCells/s throughput for 100M cells on commodity GPU; covering the DomainZoo 12-domain physical dynamics suite (9/12 pass under a deterministic re-run; failures reported) and maze navigation with 100% escape.
...
| **43-Column Cortical Array**<br>(`CorticalMacroArray`, Champion) | **43 columns / 1,032 cells / 258 inhibitory macro-axons** | Train fitness 4.478; selection set (2013–2015) Sharpe 0.76, return +16.0%, DD 7.7% | **Annualized Sharpe 0.22, Cumulative Return +13.55% (RMB 1M -> 1.135M), Max DD 20.11%, Calmar 0.67 (1,141 rebalances)** | **Positive but modest OOS**: the macro-array holds a positive OOS return; the earlier +20.30% / Sharpe +0.36 / Calmar 1.58 figures are withdrawn as not reproducible |

* **Scientific Finding**: This comparison is directional counterfactual evidence: scaling training generations on a single micro-column memorizes in-sample noise, whereas **cortical column arrays** interconnected by long-range lateral inhibitory macro-axons retain a positive, if modest, out-of-sample return (Sharpe 0.22, Calmar 0.67). The claim of "portfolio homeostasis" is not supported by the reproducible numbers and is withdrawn.
...
To verify universal control capabilities across classical and non-linear physics, SDSCC was evaluated on 12 benchmark control domains (`tasks/control/domain_zoo.hpp`), each converging in seconds and exporting to zero-heap `.bin` images. Under a deterministic re-run of `tools/train_domain_zoo.cpp`, **9 of the 12 domains pass the M1 gate; MagLev, DC Motor and Servo fail (OOD 0%, train SR 10%, train SR 0% respectively) and are reported as failures** — the earlier "12/12" claim came from an earlier substrate and is not reproducible:
