...
* **Evaluated Evidence**:
  ...
  2. **Lane-keeping cortex, shipped champion `checkpoints/adas_cortex_champion.bin` (210 cells, 630 synapses) [E1]**: 12/12 training and 4/4 holdout scenarios pass the ≤ 0.60 m *max*-CTE envelope with 100% completion; standard S-curve mean CTE 6.36 cm, gentle-S 6.03 cm, tight-curve max 3.96 cm, stop-and-go 5.15 cm, holdout validation stop-and-go 7.76 cm; generalization gap is 0.29x; straight cruise steady-state bias after L3 tuning is compressed to 3.38 cm ($0.0338 \pm 0.0015\,\text{m}$ vs baseline Stanley $0.0286 \pm 0.0017\,\text{m}$, difference only 5.2 mm); on the scenarios it wins (s_curve, curve_easy, tight_curve, highway, ramp_merge) it outperforms Stanley by 1.0–1.3x, but the overall 16-scenario tally is 7 wins / 9 losses (Table 2b); C11/Python bit-exact parity verified with max |Δ| < 1e-5 (PASS). Comparison data across seeds available in `runs/adas_champion_vs_stanley_seed7.json` and `runs/adas_champion_vs_stanley_seeds1-10.json`. **Update (2026-09-11)**: after L3 sep-CMA-ES retuning the shipped champion is 210 cells / **659 synapses** with a 16-scenario **9W / 7L** tally (`runs/adas_champion_vs_stanley_seeds1-10_tuned_20260911.json`); the tables in this section keep the 2026-09-04 audit snapshot (630 synapses / 7W-9L), and the README carries the current figures.
  3. **10.7-year, 43-asset commodity futures out-of-sample audit [E1]**: the single-column toy model collapses OOS (Sharpe -0.28 to -0.53). The 43-column paper anchor (Sharpe 0.22 / +13.55% / Calmar 0.67) **does not reproduce** under `--seed 0` (selection Sharpe 2.16, OOS **-0.03**; `quant_cortical_array_oos_repro_20260911.json`). Task-layer cold eval of the shipped `quant_cortical_array_champion.bin` (`eval_cortical_array_bin`, persisted u8/64 gains) yields OOS Sharpe **+0.14** / +6.3% / DD 25.6% — **not** the 0.22 anchor. Earlier +20.30% / Sharpe +0.36 withdrawn. Gate 3 remains **FAIL** for “reproducible 0.22”.
  4. **Full-scale hardware & physical zoo [E1]**: 100M-cell pure operator kernel throughput reaches 916.3 ~ 1,027.9 GCells/s (0.097 ~ 0.109 ms/step latency, 2.76 GB VRAM, full pipeline 14.8 ms / 6.74 GCells/s at 67.4 Hz) on a single commodity RTX 5060 (8GB); DomainZoo: **12/12** classical physical control domains pass the M1 gate under a deterministic re-run (`tools/train_domain_zoo.cpp`, `checkpoints/domain_zoo_report.json`); a mid-window 9/12 drift (maglev / dc_motor / servo) on 2026-09-09 was repaired at the Task layer and re-locked to 12/12 on 2026-09-10/11 and again on 2026-09-14 — do not cite the expired 9/12 as current; 2D maze navigation achieves 100% escape with 0 deadlocks and 0 collisions.
...
> 6. **Honest multi-asset audit with negative baselines [E1]**: 10.7-year OOS; `--seed 0` retrain Sharpe **-0.03**; shipped-bin cold eval **+0.14** (not the 0.22 anchor); do not claim regime-shift survival.
> 7. **Full-scale hardware throughput, Six Empirical Gates & physical zoo [E1]**: Establishing the Six Empirical Verification Gates protocol (baseline probe, selection convergence, OOD holdout, deterministic zero-GC, offline replay, shadow parity); achieving >1,000 GCells/s throughput for 100M cells on commodity GPU; covering the DomainZoo 12-domain physical dynamics suite (**12/12** under current deterministic re-run; mid-window 9/12 errata closed) and maze navigation with 100% escape.
...
### 5.1 Deterministic Zero-GC Microarchitecture [E1]
Reproducible measurement on standard commodity x86-64 CPUs (`tests/test_binary_runtime_scale.c`), loading `checkpoints/sdsc_mega_1million.bin` (1,000,000 cells / 3,000,000 synapses, 64 receptors / 16 effectors) via zero-copy mmap:
* **mmap Mount Time**: **5.328 ms**;
* **Full Forward Step (1M cells)**: **134.588 ms**;
* **Throughput**: **7.43 MCells/sec**;
* **Runtime Heap Allocation**: Exactly **0 bytes** (0 malloc / 0 free).
* **Withdrawn**: the earlier **19.06 ns P50 / 52.47 M-Inferences/sec / 179.0 ns P99 / 35.9 μs worst-case** figures came from `test_c11_apex_maneuver` and `include/kun/cellular/sdsc_apex_cortex.h`, both removed in commit `b555cb7`; no artifact in the current tree produces them, so they must not be cited.

### 5.2 Embodied Autonomous Driving Control Cortex: Shipped Champion Across 16 Scenarios, Stanley Baseline & C11 Parity [E1]
In our September 2026 comprehensive re-testing audit, running generational evolution from scratch using `tools/train_adas_cortex.py` (pop 16, gen 20, 210 cells, 630 synapses) achieved a **100% pass rate** across all 12 training scenarios and all 4 holdout validation scenarios; standard S-curve mean CTE was $6.36\,\text{cm}$, gentle-S $6.03\,\text{cm}$, tight-curve max $3.96\,\text{cm}$, stop-and-go $5.15\,\text{cm}$, and validation holdout stop-and-go $7.76\,\text{cm}$, with a generalization gap ratio of only $0.29\times$. Bit-exact parity between the exported C11 header and Python runtime was verified ($\max \vert \Delta \vert < 10^{-5}$, `tests/test_adas_cortex_parity.py` 2/2 PASS).

The shipped champion binary artifact `checkpoints/adas_cortex_champion.bin` (incorporating L3 parameter fine-tuning, gen 60, 210 cells, 630 synapses: 12 receptors, 192 hidden, 6 effectors) was benchmarked against an industrial standard Stanley tracking controller (integrating adaptive centripetal limits and speed feedforward) on the identical dual-track bicycle dynamics environment across all 16 scenarios under identical random noise (seed=7, reproduced in `runs/adas_champion_vs_stanley_seed7.json`):
...
* **In-Sample Prosperity vs Out-of-Sample Collapse**: In-sample (2005~2012), this single column achieved an impressive Sharpe ratio of **0.84** and **+918.8%** return. However, across the 10.7-year out-of-sample blind test (2016~2026, 2,593 trading days), it collapsed catastrophically: in a 30-generation single-asset test Sharpe fell to **-0.28** (Return **-18.02%**, Max Drawdown **31.48%**), and across the full stress test it deteriorated to **Sharpe -0.53, Return -35.72%, Max Drawdown 53.71% (FAIL)**.
...
| Architecture Variant | Cellular Complexity & Wiring | In-Sample Train (2005~2012) | 10.7-Year Out-of-Sample Test (2016~2026, 2,593 Trading Days) | Empirical Falsification Conclusion |
| :--- | :--- | :--- | :--- | :--- |
...
| **Single-Asset Overfitted Model**<br>(`FuturesQuantTask`, Rebar rb) | 14 cells / 12 synapses (single column) | Sharpe 0.84, Return +918.8% | **Sharpe -0.28 ~ -0.53, Return -18.02% ~ -35.72%, Max Drawdown 31.48% ~ 53.71%** | **Negative Baseline (FAIL)**: Severe in-sample memorization; single column cannot withstand macro regime shifts |
| **43-Asset Single Core**<br>(`train_multi_asset_quant_l3`) | 12 cells / 18 synapses (single nucleus) | Fitness 2.48, Sharpe 0.06 | **Sharpe 0.18, Return -3.86%, Max Drawdown 37.71%** | **Mediocre Baseline**: Multi-asset pooling dampened variance, but a single cellular nucleus cannot execute cross-asset dynamic hedging |
| **43-Column Cortical Array**<br>(`CorticalMacroArray`) | **43 columns / 1,032 cells** | `--seed 0` retrain val **2.16**; shipped-bin cold eval val **0.74** | Retrain OOS **-0.03**; shipped-bin cold eval OOS **+0.14** / +6.3% / DD 25.6% | **Gate 3 FAIL vs 0.22 anchor**. Retrain overfit frozen; shipped bin is now loadable but is not 0.22. |

* **Scientific Finding**: Scaling generations on a single column memorizes in-sample noise. The 43-column `--seed 0` retrain repeats the pattern (val 2.16 vs OOS **-0.03**). The shipped `champion.bin` is now cold-evaluable (OOS **+0.14**). **Do not treat unreproducible 0.22 as current.**
...
| **Pure Kernel Latency** | **0.103 ms (103 μs)** | **0.181 ms (181 μs)** | **0.097 ~ 0.109 ms (97 μs)** |
| **Kernel Compute Throughput** | **9.74 GigaCells/s** | **55.38 GigaCells/s** | **916.32 ~ 1,027.95 GigaCells/s** |
| **Full Pipeline Step Latency** | **0.142 ms** | **1.237 ms** | **14.838 ms** |
| **Full Closed-Loop Control Frequency** | **7,057 ~ 9,742 Hz (~10 kHz)** | **808 Hz** | **67.4 Hz** (Exceeds automotive 50Hz planning) |
...
* Note: The rows above are strictly **hardware throughput and memory scaling benchmarks** (`tests/test_adas_scales.cpp`, `tests/test_cuda_scale.cpp`, and `bench_adas_scales`). Pure kernel throughput (916 ~ 1,028 GCells/s) measures raw stream processor primitive evaluation rate; full pipeline latency (14.8 ms) accounts for host-device synchronization and graph scheduling. Rows on "prediction horizon" or "counterfactual foresight" from early drafts are explicitly designated as synthetic stress-test harnesses rather than closed-loop real-world perception evaluations.

### 5.5 DomainZoo 12-Domain Low-Dimensional Physical Zoo [E1]
To verify universal control capabilities across classical and non-linear physics, SDSCC was evaluated on 12 benchmark control domains (`tasks/control/domain_zoo.hpp`), each converging in seconds and exporting to zero-heap `.bin` images. Under a deterministic re-run of `tools/train_domain_zoo.cpp` (2026-09-14 lock, `checkpoints/domain_zoo_report.json`), **all 12 domains pass the M1 gate**. Errata: a 2026-09-09 mid-window drift reported 9/12 (MagLev / DC Motor / Servo); Task-layer plant/curriculum repairs restored 12/12 on 2026-09-10/11 — **do not treat the expired 9/12 as current**:
1. **CartPole**: upright within $\pm 0.02\,\text{rad}$ with cart travel limits;
2. **BallBeam**: ball settled to setpoint without sustained oscillation;
3. **Bicycle**: roll/yaw self-stabilized under forward motion;
4. **Boiler**: MIMO pressure–level cross-coupling balanced;
5. **Cruise**: zero-offset speed tracking under grade and drag steps;
6. **DC Motor**: square-wave following with rise time $< 50\,\text{ms}$ (mid-window FAIL → Task-layer PASS);
7. **MagLev**: hard hover at $1\,\text{mm}$ gap on an open-loop-unstable plant (mid-window FAIL → curriculum PASS);
8. **Rocket Hover**: soft-landing vertical speed $< 0.1\,\text{m/s}$;
9. **Servo**: deadzone-gated step response without limit-cycle chatter (mid-window FAIL → Task-layer PASS);
10. **Thermal**: ambient-step temperature error $< 0.1^\circ\text{C}$;
11. **Vibration**: resonance-peak acceleration attenuation $> 26\,\text{dB}$;
12. **Water Tank**: nonlinear coupled-tank level recovery after outlet disturbance.
...
* **Empirical Validation (withdrawn and replaced)**: The 1,024-cell / 196,608-synapse champion (`checkpoints/doudizhu_game_champion.bin`, topology verified against its `.cert.json`) was previously credited with an **82.5%** out-of-distribution win rate; **that figure is retracted** — its training and test environment was a simplified synthetic simulation (no bidding, no legal-play validation, random-number opponents), which has no bearing on real three-player DouDiZhu. The current reported baseline is the U0.2 champion `checkpoints/doudizhu_cand_scorer.bin` (82 cells / 425 synapses): **57.0% win rate (1139/2000 games, Wilson 95% lower bound 54.8%)** against heuristic opponents under a full rules engine (landlord 61.2% / peasant 56.8%). The previously reported **411.8 μs** single-step latency is likewise withdrawn, as its measurement artifact is no longer in the tree.

---

## 6. Threats to Validity and Limitations

1. **Withdrawn headline claims**: the 0.0075 m / 0.042 m CTE and the "ISO 26262 ASIL-D Compliant" string of a previous draft came from an unreproducible report file (`runs/flowengine_3d_grand_benchmark_report.json`, no generating script); the **19.06 ns P50 / 52.47 M-Inferences/sec** micro-benchmark came from `test_c11_apex_maneuver` and `sdsc_apex_cortex.h`, both removed in commit `b555cb7` and absent from the current tree. Tables 2/2b now use the shipped champion only, and §5.1 reports only what `tests/test_binary_runtime_scale.c` reproduces. Nothing in this paper is a functional-safety certification; all driving results are simulation-only.
...
12. **Quant macro-array and DomainZoo errata**: +20.30% / Sharpe +0.36 withdrawn; paper-anchor Sharpe 0.22 **does not reproduce** (`--seed 0` OOS **-0.03**). Shipped-bin cold eval OOS **+0.14**, not 0.22. DomainZoo current lock is **12/12** (§5.5).
13. **Topological Causal Significance & Ablation Baselines**: Inspired by connectome ablation protocols [29], task-layer **degree-preserving random rewiring** and **E/I sign flipping** are now in `tools/ablate_topology_prior.py` (stability campaign #4). Both the L3 lock and the stadium-adapted ADAS bins **PASS**: rewiring inflates cost 34.1× / 23.9× (0/8 completions); sign flip 10.8× / 22.0×. Evidence: `runs/adas_topology_ablation_20260914.json`. ADAS-only so far.
