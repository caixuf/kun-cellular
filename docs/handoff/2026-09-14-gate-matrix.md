...
> **2026-09-15 冻结**：结案 `2026-09-15-cellular-closeout.md`。本矩阵不再为填格扩测。
...
- **Gate 1 (基线探针 / M1 Gate)**：`PASS` *(冻结 JSON 12/12)*；**隔离冷评不得写成 12/12**。`ZooTask::reset` 已清 `o_[]`，`reset_state` 已还原 `membrane_pores`，隔离测试已把 OOD `max_steps_` 与 clone 评测路径对齐到 `evaluate_organism`。cartpole ID/OOD：JSON 0.9 / leaky=clone=fresh **0.1**。ballbeam 隔离 OOD **0.3**（旧 0.4 是短地平线 + 手写 clone 循环的口径差，不是隔离可复现）。M1 leaky 与 clone 均为 **10/12**。未改 `domain_zoo_report.json`。
...
- **Gate 6 (影子对账)**：`FAIL`。`tests/test_flow_domain_zoo_gate6_shadow.cpp` 负例锁档：抖动 **8/12**（阈值 0.80）。振颤域 rocket 2.39 / thermal 3.07 / servo 1.95 / bicycle 2.23。cartpole 隔离生存 **1/10**（PD 专家 10/10）。ctest 绿只表示审计数字锁住，不是 G6 过关。未覆盖 bin。
...
# DomainZoo Gate 5 12 域回放 + Gate 6 抖动审计 + 评测隔离冷评
