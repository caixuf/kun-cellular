...
> **2026-09-11 收口**：幅度/递归消融与路线 A 已完成。结构化未能翻盘；**`io_mix` 定为塑关默认**；`io_evo` 负结果。详见 `docs/handoff/2026-09-11-fieldcml-core-closed-task-campaign.md`。

1. ~~定位「结构化为何输」~~ → **已裁决**：走随机 IO 混合先验，不雕局部 DAG。
2. **任务层战役**（当前主线）：DomainZoo 锁档 → household → ADAS 多种子 → …
3. H7 / 64K 放大 / GPU eval：**后置**；无新机制前不冲规模。
4. 若再攻「可演化接线」：仅允许 **间接编码/超参** 新预注册，禁止复读广谱 `io_evo`。
