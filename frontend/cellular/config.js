...
export const TOOLTIP_DICT = {
  ...
  'st-real-cells': { title: '真实渲染细胞 (Rendered)', desc: '后端实际生成并驱动 3D 场景/前向计算的物理细胞实体数，与活跃细胞数同源同值。', hint: '放大视距时按像素密度自动实化为实体细节' },
  'st-universe-rad': { title: '宇宙物理占据半径 (Cosmic Radius)', desc: '当前生命体在 1000m 算力宇宙中真实的物理分布半径。基于宿主硬件算力上限按立方根正比映射。', hint: '小生命体紧凑凝聚，亿级生命体漫布全宇宙' },
  'st-universe-vol': { title: '算力宇宙体积占比 (Volume Ratio)', desc: '当前生命体规模占当前硬件显存与算力极限容量 (1.78×10⁸ 细胞) 的物理体积占比 V ∝ N / N_max。', hint: '12细胞反射弧占比0.000007%，1亿细胞世界模型占比55.88%' },
  'lod-organ': { title: '器官微观视图 (Organ 30)', desc: '以高精细生物质膜动画观察 30 个核心决策细胞的呼吸与放电脉冲。', hint: '适合初学者清晰观察信号传递因果' },
  'lod-1m': { title: '宏观星系流形 (Macro Manifold)', desc: 'GPU 单批次点云渲染当前生命体的宏观形态，规模即后端真实细胞数；放大局部时按像素密度实化为实体细胞。', hint: '点击领略宏观星系级流形视觉' },
  'act-buy': { title: '买开 / 加速信号 (ACT_POS)', desc: '经过层层神经计算后，大脑输出的主动进攻/加速行为强度。', hint: '数值大于 0.2 时高亮发光' },
  'act-sell': { title: '卖开 / 刹车信号 (ACT_NEG)', desc: '大脑输出的主动防御/避险行为强度。', hint: '数值大于 0.2 时高亮发光' },
  'pos-status': { title: '当前持仓 / 姿态控制', desc: '经动作执行器平滑滤波后的净持仓或方向舵偏角状态。', hint: '多头=正，空头=负，空仓=0' },
  'pnl-card': { title: '实时累计盈亏 (PnL)', desc: '大脑策略在真实行情推演中的累计账户净收益。衡量适应度核心指标。', hint: '代数越高，长期胜率和收益稳定性越强' },
  'btn-pulse': { title: '注入电位脉冲 (Inject Pulse)', desc: '向 4 个感知受体人为注入一个标准瞬态生物电位刺激，观察信号如何在皮层微柱中逐层前向扩散。', hint: '快捷键: P' },
  'btn-glut': { title: '谷氨酸过载风暴 (Glutamate Storm)', desc: '模拟神经递质极端过载，全体细胞膜电位剧烈震颤放电。', hint: '快捷键: G' },
  'btn-gaba': { title: 'GABA 抑制复位 (GABA Reset)', desc: '释放超极化抑制递质，瞬间平息全脑癫痫级过载，重置为基线静息电位。', hint: '快捷键: R' },
  'btn-mitosis': { title: '诱发有丝分裂 (Induce Mitosis)', desc: '在一条活跃突触上分裂诞生一个新代谢运算细胞，提升回路复杂度。', hint: '快捷键: M' },
  'btn-rewire': { title: '随机突触重连 (Rewire Synapse)', desc: '随机打断并重连一条突触连接，尝试建立新的跨区域因果通路。', hint: '快捷键: W' },
  'btn-apop': { title: '诱导细胞凋亡 (Apoptosis)', desc: '剪枝清除对最终决策无贡献的孤立/冗余细胞，实现网络拓扑极简化。', hint: '快捷键: A' },
  'btn-focus': { title: '镜头对焦核心区 (Focus Core)', desc: '平滑推进镜头，特写聚焦至正在进行因果计算的核心决策微柱。', hint: '双击任意细胞可精准聚焦该细胞' },
  'btn-pause': { title: '冻结时间切片 (Pause / Resume)', desc: '将当前大脑的电位推演和突触脉冲冻结在当前瞬间，便于静态剖析回路。', hint: '快捷键: 空格 Space' }
};
