...
// ============================================================================
// 重要注记：本基准测试仅用于 CPU 微架构层面的单步前向推演时延 (ns) 与访存吞吐对照，
// 非机器学习任务级真实 SOTA 对照（测试中 MLP 为固定权重探针，NEAT 为静态手写拓扑，
// 且运行于伪随机游走序列）。论文正文已严格将此测试与实际算法性能对照剥离。
// ============================================================================
...
// ============================================================================
// 4. 28 种计算细胞全谱系零 GC 与确定性推断基准实测
// ============================================================================
void test_24_primitives_forward_latency_and_zero_gc() {
    std::cout << "[Test 4] 运行 28 种计算细胞全谱系确定性推断与零 GC 内存基准实测...\n";
    ...
    std::cout << "  -> 28 种计算细胞全谱系确定性推断与零 GC 实测 100% 满分通过!\n";
}
