...
static inline void require(bool cond, const char* msg) {
    if (!cond) {
        std::cerr << "FATAL: requirement failed: " << msg << std::endl;
        std::abort();
    }
}
...
void test_checkpoint_preservation_without_genetic_pollution() {
    ...
    require(!org.synapses.empty(), "organism synapses must not be empty");
    ...
    require(org.save_checkpoint_json(ckpt_path), "save_checkpoint_json must succeed");

    CellularOrganism reloaded = CellularOrganism::load_checkpoint_json(ckpt_path);
    require(!reloaded.synapses.empty(), "reloaded synapses must not be empty");
    
    // 验证：加载后保持了个体后天记忆 (weight == 2.5)
    std::cout << "  ↳ 加载后突触实时记忆权重: " << reloaded.synapses[0].weight << " (期望 2.5)\n";
    require(std::abs(reloaded.synapses[0].weight - 2.5) < 1e-4, "reloaded live weight mismatch");

    // 验证：加载后没有篡改先天遗传基线 (initial_weight == 1.0)
    std::cout << "  ↳ 加载后先天遗传初始基线: " << reloaded.synapses[0].initial_weight << " (期望 1.0)\n";
    require(std::abs(reloaded.synapses[0].initial_weight - 1.0) < 1e-4, "reloaded initial weight mismatch");
    ...
    require(std::abs(offspring.synapses[0].weight - 1.0) < 1e-4, "offspring weight reset mismatch");
...
void test_baldwin_crystallization_inheritance() {
    ...
    CellularOrganism org = CellularOrganism::create_seed_organism();
    require(!org.synapses.empty(), "org synapses must not be empty");
    org.synapses[0].initial_weight = 1.0;
    ...
    require(std::abs(org.synapses[0].initial_weight - 1.75) < 1e-4, "crystallized initial weight mismatch");

    // 存储并重新加载
    std::string ckpt_path = "/tmp/flow_baldwin_test_checkpoint.json";
    require(org.save_checkpoint_json(ckpt_path), "baldwin save_checkpoint_json must succeed");
    CellularOrganism reloaded = CellularOrganism::load_checkpoint_json(ckpt_path);
    require(!reloaded.synapses.empty(), "reloaded baldwin synapses must not be empty");

    require(std::abs(reloaded.synapses[0].initial_weight - 1.75) < 1e-4, "reloaded initial weight mismatch");
    require(std::abs(reloaded.synapses[0].weight - 1.75) < 1e-4, "reloaded live weight mismatch");
    ...
    require(std::abs(offspring.synapses[0].weight - 1.75) < 1e-4, "offspring inherited weight mismatch");
...
void test_recurrent_synapse_restoration() {
    ...
    CellularOrganism org = CellularOrganism::create_seed_organism();
    require(org.synapses.size() >= 2, "org must have at least 2 synapses");
    org.synapses[1].is_recurrent = true;
    org.synapses[1].hebbian_decay = 0.045;
    org.compile();

    std::string json_str = org.to_json();
    require(json_str.find("\"recurrent\": true") != std::string::npos, "missing recurrent tag in json");
    require(json_str.find("\"hebb_decay\": 0.045") != std::string::npos, "missing hebb_decay tag in json");

    CellularOrganism reloaded = CellularOrganism::from_json(json_str);
    require(reloaded.synapses.size() >= 2, "reloaded must have >= 2 synapses");
    require(reloaded.synapses[1].is_recurrent == true, "reloaded is_recurrent mismatch");
    require(std::abs(reloaded.synapses[1].hebbian_decay - 0.045) < 1e-4, "reloaded hebbian_decay mismatch");
...
void test_stratified_5_tier_export_contract() {
    ...
    CellularOrganism org = CellularOrganism::create_seed_organism();
    require(!org.synapses.empty() && !org.cells.empty(), "organism must have cells and synapses");
    org.synapses[0].initial_weight = 1.0;
    ...
    require(genome_str.find("\"initial_weight\": 1") != std::string::npos, "initial_weight in genome.json");
    ...
    require(!from_gene.synapses.empty(), "from_gene synapses not empty");
    require(std::abs(from_gene.synapses[0].weight - 1.0) < 1e-4, "from_gene live weight mismatch");
    ...
    require(epi_str.find("\"drift\": 2.2") != std::string::npos, "epigenome drift mismatch");
    require(epi_str.find("\"fitness_score\": 99.5") != std::string::npos, "epigenome fitness mismatch");
    ...
    require(rt_str.find("\"output\": 0.88") != std::string::npos, "runtime output mismatch");
    require(rt_str.find("\"activations\": 42") != std::string::npos, "runtime activations mismatch");
    require(rt_str.find("\"weight\": 3.2") != std::string::npos, "runtime weight mismatch");
