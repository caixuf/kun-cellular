// ============================================================================
// train_locomotion_gait.cpp — 多足步态生命体训练器 (organism 真前向)
//
// 流程: 随机基线 -> 形态发生演化 -> Train/Holdout-ID/Holdout-OOD 三隔离门禁
// OOD = 更强重力(0.42) + 更低地面摩擦(0.12) —— 跨物理参数泛化
//
// 编译: g++ -O3 -march=native -std=c++20 -I include tools/train_locomotion_gait.cpp ...
// ============================================================================
...
int main() {
    ...
    std::printf("==========================================================\n");
    std::printf("  多足步态生命体 · organism 真前向演化 (C++20 Native)\n");
    std::printf("==========================================================\n");
    ...
    LocomotionGaitTask train_env, id_env;
    LocomotionGaitTask::Params ood_p;
    ood_p.gravity = 0.33;
    ood_p.ground_friction = 0.23;
    LocomotionGaitTask ood_env(ood_p);
    ...
    split.task_name = "LocomotionGait";
    ...
    champion.save_checkpoint_bin("checkpoints/locomotion_gait_champion.bin");
    std::ofstream rf("checkpoints/locomotion_gait_report.json");
    rf << report.to_json();
    rf.close();
