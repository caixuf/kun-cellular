#!/usr/bin/env python3
"""
SDSCC Apex Autonomous Driving Cortex C11 Code Generator
=======================================================
Exports the 5-column Apex Driving Brain (Centering, Left-Turn, Right-Turn,
Multi-point U-Turn, Prefrontal Game-Theory Hub) into a self-contained,
zero-malloc, zero-GC ISO C11 kernel: `sdsc_apex_cortex.h`.
"""
...
def main():
    ...
    target_path = os.path.join(root_dir, "include", "kun", "cellular", "sdsc_apex_cortex.h")
    flowengine_target = "/home/caixuf/code/FlowEngine/include/sdsc_apex_cortex.h"

    with open(target_path, "w", encoding="utf-8") as f:
        f.write(APEX_HEADER)
    print(f"Exported Apex C11 cortex header to: {target_path}")

    if os.path.exists(os.path.dirname(flowengine_target)):
        with open(flowengine_target, "w", encoding="utf-8") as f:
            f.write(APEX_HEADER)
        print(f"Synchronized Apex C11 cortex header to FlowEngine: {flowengine_target}")
